...
*/
  function get_uuid() {
    ...
    // test for and save the callback
    if((a_l == 0) || (typeof a_[a_l - 1] != 'function')) { return; };
    var callback = a_[a_l - 1];
    // make sure other args are numbers
    for(var i=0; i < (a_l - 1); i++) {
      if(typeof a_[i] != 'number') { callback('arg NaN'); return; };
    };
    switch(a_l) {
      case 1:   // just the callback. return a single string
        relax( '_uuids' , function(err, body) { 
          if(!err) { callback(null, body.uuids[0]); }
          else { callback(err); };
        } ); break;
      case 2:  // one arg plus callback, return an array
        var query='_uuids?count='+a_[0];
        //I couldn't figure out how to pass count into relax.opts...
        relax( query , function(err, body) { 
          if(!err) {
            callback(null, body.uuids);
          } else { callback(err) };
        } ); break;
      default:  // at least 2 args plus callback; return an array of arrays
        var ids = [ ];    var total = 0;
        for( var i=0; i < (a_l - 1); i++ ) { 
          ids.push( [ ] );    // set up the skeleton of the 2d array to return
          total += a_[ i ];   // we'll only run one query
        };    
        var query='_uuids?count='+total;
        relax( query , function(err, body) {
          if(!err) {
            for( i=0; i < (a_l - 1); i++ ) {
              for(j=0; j < a_[i]; j++) { ids[i].push(body.uuids.pop()) };
            };
            callback(null, ids);
          } else { callback(err) };
        } ); break;
